    ...
    <div className="flex flex-col bg-slate-100/90 dark:bg-slate-800/95 backdrop-blur-sm rounded-xl overflow-hidden border border-slate-200 dark:border-slate-700 min-w-[280px] sm:w-[300px] max-w-full transition-colors shadow-sm">
      ...
      >
        ...
        {tasks.length === 0 && (
          <div
            onClick={() => onAddTask(id)}
            className="flex flex-col items-center justify-center border-2 border-dashed border-slate-300 dark:border-slate-600 rounded-xl text-slate-400 dark:text-slate-500 text-xs italic py-8 min-h-[100px] cursor-pointer hover:border-slate-400 dark:hover:border-slate-500 transition-colors gap-2"
          >
        ...
        {/* Add new task button at bottom of every column */}
        <button
          onClick={() => onAddTask(id)}
          className="w-full mt-1 py-2 px-3 flex items-center gap-2 text-xs font-medium text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 hover:bg-slate-200/60 dark:hover:bg-slate-700/50 rounded-lg transition-colors cursor-pointer"
        >
