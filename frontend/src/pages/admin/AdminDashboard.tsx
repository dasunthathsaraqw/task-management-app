...
export const AdminDashboard: React.FC = () => {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      const tasksData = await getAllTasks({ limit: 5, sort: "createdAt", order: "desc" });
      ...
      allTasksData.tasks.forEach((t) => {
        t.statusHistory.forEach((hist) => {
          logs.push({
            ...
            operator: typeof hist.updatedBy === "object" ? (hist.updatedBy as any).username : "Admin",
          });
        });
      });
  ...
  if (loading || !summary) {
    return (
      <div className="h-96 flex flex-col items-center justify-center space-y-3">
        ...
        <span className="text-slate-500 text-sm font-medium">Analyzing system analytics...</span>
      </div>
    );
  }
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm">Real-time summaries, metrics, and operations overview.</p>
        </div>
        <div>
          <Button onClick={() => setCreateOpen(true)} className="flex items-center space-x-1.5 shadow-sm">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-5">
        ...
        <div className="bg-white border border-slate-200 p-5 rounded-2xl shadow-sm hover:shadow-md hover:border-slate-300 transition-all flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-2xl font-bold text-slate-800 mt-0.5">{summary.totalTasks}</p>
          </div>
        </div>
        ...
        <div className="bg-white border border-slate-200 p-5 rounded-2xl shadow-sm hover:shadow-md hover:border-slate-300 transition-all flex items-center space-x-4">
          ...
          <div>
            <p className="text-slate-450 text-xs font-semibold uppercase tracking-wider">Open</p>
            <p className="text-2xl font-bold text-slate-800 mt-0.5">{summary.statusBreakdown.Open}</p>
          </div>
        </div>
        ...
        <div className="bg-white border border-slate-200 p-5 rounded-2xl shadow-sm hover:shadow-md hover:border-slate-300 transition-all flex items-center space-x-4">
          <div className="p-3 bg-amber-100/60 rounded-xl text-amber-600">
            <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 1121.21 12H16.5" />
            </svg>
          </div>
          <div>
            <p className="text-slate-450 text-xs font-semibold uppercase tracking-wider">In Progress</p>
            <p className="text-2xl font-bold text-slate-800 mt-0.5">{summary.statusBreakdown["In Progress"]}</p>
          </div>
        </div>
        ...
        <div className="bg-white border border-slate-200 p-5 rounded-2xl shadow-sm hover:shadow-md hover:border-slate-300 transition-all flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-2xl font-bold text-slate-800 mt-0.5">{summary.statusBreakdown.Done}</p>
          </div>
        </div>
        ...
        <div className="bg-white border border-slate-200 p-5 rounded-2xl shadow-sm hover:shadow-md hover:border-slate-300 transition-all flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-2xl font-bold text-slate-800 mt-0.5">{summary.totalUsers}</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 space-y-4">
          <h3 className="text-sm font-semibold text-slate-800 uppercase tracking-wider">Status Distribution</h3>
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 space-y-4">
          <h3 className="text-sm font-semibold text-slate-800 uppercase tracking-wider">Priority Density</h3>
          <div className="space-y-3 pt-2">
            {Object.entries(summary.priorityBreakdown).map(([priority, count]) => {
              const pct = Math.round(((count as number) / maxPriorityVal) * 100);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl shadow-sm border border-slate-200 p-6 flex flex-col justify-between space-y-4">
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-sm font-semibold text-slate-800 uppercase tracking-wider">Recent Tasks</h3>
              <Link to="/admin/tasks" className="text-xs font-semibold text-purple-650 hover:text-purple-750 transition-colors">
                View All Tasks →
              </Link>
            </div>
                ...
                <tbody className="divide-y divide-slate-50 text-xs text-slate-700">
                  {recentTasks.map((task) => (
                    <tr key={task._id} className="hover:bg-slate-50/40">
                      ...
                      <td className="py-3">
                        <span className="font-medium text-slate-500">{task.priority}</span>
                      </td>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 space-y-4">
          <h3 className="text-sm font-semibold text-slate-800 uppercase tracking-wider">System Activity Feed</h3>
          <div className="space-y-4 overflow-y-auto max-h-[260px] pr-1">
            {activityLogs.length === 0 ? (
              <p className="text-slate-450 text-xs italic">No system activities recorded yet.</p>
            ) : (
                ...
                <div key={idx} className="flex items-start space-x-2.5 text-xs">
                  ...
                  <div className="flex-1 space-y-0.5">
                    <p className="text-slate-700 leading-normal">
                      <span className="font-semibold text-slate-800">{log.operator}</span> updated status of{" "}
                      <Link to={`/admin/tasks/${log.taskId}`} className="font-medium text-purple-650 hover:underline">
                        {log.taskTitle}
                      </Link>{" "}
                      to <span className="font-semibold text-slate-800">{log.status}</span>
                    </p>
